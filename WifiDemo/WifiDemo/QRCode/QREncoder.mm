...
+ (NSData*)AESEncryptString:(NSString*)string withPassphrase:(NSString*)passphrase {
    if (passphrase.length>kCCKeySizeAES256) {
        throw [NSException exceptionWithName:@"invalid passphrase exception" reason:[NSString stringWithFormat:@"passphrase too long: %ld", passphrase.length] userInfo:nil];
    }
...
+ (NSImage*)renderDataMatrix:(DataMatrix*)matrix imageDimension:(int)imageDimension withBgColor:(uint32_t)bg withCodeColor:(uint32_t)cg {
    ...
    const uint32_t white = bg, black = cg, transp = 0x00FFFFFF;
